...
import (
	...
	models "github.com/Yogeshlodhi/Models"
...
var (
	...
	ErrCantButyCartItem   = errors.New("cant update the purchase")
)
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	// Push items from UserCart to the order_list of the latest order
	filter2 := bson.D{primitive.E{Key: "_id", Value: id}}
	update2 := bson.M{"$push": bson.M{"orders.$[order].order_list": bson.M{"$each": getcartitems.UserCart}}}

	// Define array filter to target the latest order (you may need to adjust this logic based on your requirement)
	arrayFilters := options.Update().SetArrayFilters(options.ArrayFilters{
		Filters: []interface{}{bson.M{"order._id": getcartitems.Order_Status[len(getcartitems.Order_Status)-1].Order_ID}},
	})

	_, err = userCollection.UpdateOne(ctx, filter2, update2, arrayFilters)
	if err != nil {
		log.Println(err)
		return err
	}
